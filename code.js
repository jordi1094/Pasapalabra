...
function Responder(){
    let respuestaActual =displayRespuesta.value.toUpperCase()
    let circuloLetra = document.getElementById(preguntas[contador].letra)
    if(respuestaActual === preguntas[contador].respuesta){
        circuloLetra.style.backgroundColor = "green"
    }if(respuestaActual !== preguntas[contador].respuesta && respuestaActual !== "PASAPALABRA"){
    ...
    if(preguntas.length === 0){
        preguntaActual = "Fin del juego. ¡Muchas gracias por jugar!"
        botonResponder.style.opacity = "0%"
        displayRespuesta.style.opacity = "0%"
        displayPregunta.style.height = "25vh"
        displayPregunta.innerHTML = preguntaActual
    }else{    preguntaActual = preguntas[contador].pregunta;
        displayPregunta.innerHTML = preguntaActual
            displayRespuesta.value = ""}
};